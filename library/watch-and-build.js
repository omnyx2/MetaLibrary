...
// // Function to watch and build/serve only the directories that match the pattern
// const watchAndServe = (dir) => {
//   const pagesDir = path.resolve(baseDir, dir, 'pages');
//   if (fs.existsSync(pagesDir) && fs.statSync(ppagesDir).isDirectory()) {
//     console.log(`Watching ${pagesDir} for changes...`);
//     chokidar.watch(pagesDir, { ignoreInitial: true }).on('addDir', (dirPath) => {
//       const dirName = path.basename(dirPath);
//       buildAndServe(dirName);
//     }).on('error', (error) => {
//       console.error(`Watcher error in ${dir}: ${error.message}`);
//     });
//   }
// };
